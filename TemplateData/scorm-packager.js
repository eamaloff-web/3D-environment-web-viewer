...
<body>
  ...
  <script>
    ...
    createUnityInstance(canvas, config, (progress) => {
      ...
      // Send the relative path of the GLB model
      unityInstance.SendMessage("SCORMController", "ReceiveModelPath", "model.glb");
...
function downloadSCORMPackage() {
  ...
  var totalFiles = Object.keys(fetchMap).length + 1;
  var loadedCount = 0;
  var fileBlobs = {};

  function fetchFile(path, url) {
    updateProgressOverlay("Downloading Assets", "Fetching " + getFilename(url) + " (" + (loadedCount + 1) + " of " + totalFiles + ")...");
    return fetch(url)
      .then(response => {
        if (!response.ok) {
  ...
  sequence.then(() => {
    ...
    zip.file("annotations.json", JSON.stringify(sortedAnnotations, null, 2));
